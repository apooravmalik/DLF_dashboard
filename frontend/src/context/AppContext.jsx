          ...
          `${config.API_BASE_URL}/api/dashboard/charts`,
          {
            "charts": [
              ...
              },
              {
                ...
                "queries": {
                  "overview_query": {
                    "name": "Overview of Camera Status",
                    "query": "SELECT COUNT (*) AS 'Total', SUM (CASE WHEN ConnectionStatus = 1 THEN 1 ELSE 0 END) AS 'Online', SUM (CASE WHEN ConnectionStatus = 0 THEN 1 ELSE 0 END) AS 'Offline' FROM Veracity_Middleware_DB.dbo.NVRChannels JOIN TEST.dbo.Device_TBL ON dvcNvrChannel_FRK = ID JOIN TEST.dbo.NVRChannel_TBL ON NVRChannel_PRK = dvcNvrChannel_FRK JOIN TEST.dbo.Camera_TBL ON Camera_PRK = nchCamera_FRK JOIN TEST.dbo.GeoRollupCameraLink_TBL ON Camera_PRK = gclCamera_FRK",
                    "legends": ["Total", "Online", "Offline"]
                  },
                  "drill_down_query": {
                    "name": "Camera Status Building Wise",
                    "query": "SELECT bldBuildingName_TXT AS 'Building Name', SUM(CASE WHEN ConnectionStatus = 1 THEN 1 ELSE 0 END) AS 'Online', SUM(CASE WHEN ConnectionStatus = 0 THEN 1 ELSE 0 END) AS 'Offline' FROM Veracity_Middleware_DB.dbo.NVRChannels JOIN vtasData.dbo.Device_TBL ON dvcNvrChannel_FRK = ID JOIN vtasData.dbo.NVRChannel_TBL ON NVRChannel_PRK = dvcNvrChannel_FRK JOIN vtasData.dbo.Camera_TBL ON Camera_PRK = nchCamera_FRK JOIN vtasData.dbo.GeoRollupCameraLink_TBL ON Camera_PRK = gclCamera_FRK JOIN vtasData.dbo.Building_TBL ON Building_PRK = gclBuilding_FRK WHERE gclZone_FRK = 3 GROUP BY bldBuildingName_TXT ORDER BY [Building Name]",
                    "legends": ["Building Name", "Online", "Offline"]
                  },
                  "report_query": {
                    "name": "Camera Status Building Wise Report",
                    "query": "SELECT NVR.Name AS 'NVR Name', NVRChannels.Name AS 'Camera Name', ChannelNumber AS 'Channel Number', CASE WHEN NVRChannels.ConnectionStatus = 1 THEN 'Online' WHEN NVRChannels.ConnectionStatus = 0 THEN 'Offline' END AS 'Camera Status', bldBuildingName_TXT AS 'Building Name' FROM Veracity_Middleware_DB.dbo.NVRChannels JOIN Veracity_Middleware_DB.dbo.NVR ON NVR.ID = NVRID JOIN vtasData.dbo.Device_TBL ON dvcNvrChannel_FRK = NVRChannels.ID JOIN vtasData.dbo.NVRChannel_TBL ON NVRChannel_PRK = dvcNvrChannel_FRK JOIN vtasData.dbo.Camera_TBL ON Camera_PRK = nchCamera_FRK JOIN vtasData.dbo.GeoRollupCameraLink_TBL ON Camera_PRK = gclCamera_FRK JOIN vtasData.dbo.Building_TBL ON Building_PRK = gclBuilding_FRK WHERE gclZone_FRK = 3 ORDER BY NVR.ID"
                  }
                }
              },
              {
