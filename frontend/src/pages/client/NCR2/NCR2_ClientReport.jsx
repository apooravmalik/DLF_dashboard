...
const NCR2_ClientReport = () => {
  const { state } = useLocation();
  const { reportData, label, chartIndex, status } = state || {};
  ...
  return (
    <div className="min-h-screen bg-[#33414C] text-white px-4 pt-3">
      <div className="flex justify-between items-center mb-4">
        ...
        initialFilters={{
          "Building Name": label || "",
          "Status": status || "", // status is actually "Camera Status"
        }}
      />
    </div>
  );
...
export default NCR2_ClientReport;
